...
validate_internal_id(<<Bin/binary>>, #state{validate_id_regexp=undefined} = State) ->
	{ok, RegExp} = re:compile("^[a-zA-Z0-9-_]+@[a-zA-Z0-9_-]+#[0-9]+"),
	validate_internal_id(Bin, State#state{validate_id_regexp=RegExp});

validate_internal_id(<<Id/binary>>, #state{validate_id_regexp=RegExp} = State) ->
