...
join_area() ->
    ...
    libtree.srv:join_area(Node),
    libenv.srv:join_area(Node),
    libsave.srv:join_area(Node),
    libstd.srv:monsrv_rpc(monsrv, register_node, [node()]),
    Node.
...
find_obj(Key) when is_reference(Key) ->
    case libstd.srv:get_obj(Key) of
            {ok, Obj} ->
                Obj;
            {error, _Reason} ->
                case get_obj(Key) of
...
lookup_neighbours(AreaId) ->
    lookup_neighbours(AreaId, libstd.srv:neighbours()).
